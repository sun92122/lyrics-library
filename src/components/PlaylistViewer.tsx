...
export const PlaylistViewer: React.FC<PlaylistViewerProps> = ({ availableSongs }) => {
    ...
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
        ...
        <div className="flex items-center justify-between text-xs text-slate-500 pt-4 border-t border-slate-100">
          ...
          <span className="text-slate-400">點擊曲目直接開啟即時調性歌詞與和弦</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {payload.s.map((item, index) => {
          ...
          const songUrl = `/songs/${item.id}?key=${encodeURIComponent(customKey)}`;
            ...
            >
              ...
              {flow.length > 0 && (
                <div className="space-y-1.5 pt-2 border-t border-slate-100">
                  <div className="text-xs font-semibold text-slate-500">敬拜段落流程：</div>
                  <div className="flex flex-wrap items-center gap-1.5">
                    {flow.map((tag, tIndex) => (
                      <React.Fragment key={tIndex}>
                        <span className="px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-lg text-xs font-bold border border-indigo-100/80">
                          {tag}
                        </span>
              ...
              {item.note && (
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/60 text-xs text-slate-700 flex items-start gap-2">
                  ...
                  <div>
                    <span className="font-semibold text-slate-900">司琴/敬拜備註：</span>
